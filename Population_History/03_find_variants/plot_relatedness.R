...
#open the gds file
genofile <- snpgdsOpen("/scratch/tphung3/Kenya_Selection_and_Demography/Population_History/03_find_variants/filtered_variants/autosomes.gds")
...
snpset <- snpgdsLDpruning(genofile, ld.threshold=0.2,autosome.only = T)
head(snpset)
...
ibd.robust <- snpgdsIBDKING(genofile,autosome.only = T,remove.monosnp = T,missing.rate = 0.2, maf=0.05,snp.id=snpset.id) # removing monomorphing snps; adding ld pruning; this tightens it up a lot
#sink()
names(ibd.robust)
# close genofile
snpgdsClose(genofile)
# IBD analysis (KING method of moment) on genotypes:
#   Excluding 23,471,804 SNPs (non-autosomes or non-selection)
# Excluding 328,352 SNPs (monomorphic: TRUE, MAF: 0.05, missing rate: 0.2)
# Working space: 82 samples, 46,634 SNPs
...
p3b <- ggplot()+
  ...
  geom_point(data=dat,aes(x=IBS0, y=kinship),alpha=0.3)+
  xlab("Proportion of Zero IBS")+
  ylab("Estimated Kinship Coefficient (KING-robust)")+
  theme_bw()+
  ggtitle(paste("King Relatedness Results based on ",as.character(length(ibd.robust$snp.id))," LD Pruned SNPs, maf cutoff (0.05), no monomorphic",sep=""))+
