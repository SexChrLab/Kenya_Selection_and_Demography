...
rule all:
    input:
        ...
        expand("sfs_nre/chr{chrm_n}_sfs.txt", chrm_n=config["autosomes"])
...
rule generate_sfs_autosomes_nre:
    input:
        os.path.join(config["vcf_directory"], "chr{chrm_n}/chr{chrm_n}.gatk.called.vqsr.select.variants.hwe.recode.pilot.masks.nre.neutral.rmA7A11.recode.vcf.gz")
    output:
        "sfs_nre/chr{chrm_n}_sfs.txt"
    params:
        script = config["generate_sfs_script"]
    shell:
        """
